...
## 🎯 **KRITISCHE SYSTEMREGELN - CLAUDE MUSS BEFOLGEN** ⚠️

### 📚 **DOKUMENTATIONSPFLICHT - MDX DOCS & CLAUDE.MD** (MANDATORY)

**🚨 CLAUDE MUSS IMMER:**

1. **MDX DOCS AKTUALISIEREN** (NICHT NEUE MD ERSTELLEN!)
   - ✅ **EXISTIERENDE** `.mdx` Dateien im Projekt nutzen
   - ✅ **AKTUALISIEREN** mit neuen Informationen
   - ✅ **ERSTELLEN** nur wenn absolut keine MDX Docs existieren
   - ✅ **INTEGRIEREN** in bestehende Projektstruktur

2. **CLAUDE.MD IMMER AKTUELL HALTEN**
   - ✅ **JEDES PROJEKT** muss `claude.md` haben
   - ✅ **JEDE ÄNDERUNG** wird sofort dokumentiert
   - ✅ **JEDE ERKENNTNIS** wird festgehalten
   - ✅ **JEDE ENTSCHEIDUNG** wird erklärt
   - ✅ **JEDE LÜCKE** wird geschlossen

3. **DOKUMENTATIONSSTRUKTUR**
   ```
   📁 Projekt/
   ├── claude.md (PFLICHT - Immer aktuell!)
   ├── docs/
   │   ├── architecture.mdx
   │   ├── api.mdx
   │   ├── components.mdx
...
### 🔐 **SECRET MANAGEMENT - DREIFACHE SYNCHRONISATION** (MANDATORY)

**🚨 CLAUDE MUSS IMMER:**

1. **SECRETS IN .CLAUDE SPEICHERN** (Sicherer Ort)
   - ✅ **ENCRYPTED** in `.claude/` directory
   - ✅ **DOCUMENTED** mit Usage-Beispielen
   - ✅ **VERSIONED** mit Checkpoints

2. **SECRETS IN VERCEL ENVIRONMENTS UPLOADEN** (AUTO-SYNC)
   - ✅ **JEDES SECRET** aus `.claude/` → Vercel Environment
   - ✅ **PRODUCTION** Environment
   - ✅ **PREVIEW** Environment
   - ✅ **DEVELOPMENT** Environment
   - ✅ **VERCEL CLI**: `vercel env add <KEY> <VALUE>`

3. **SECRETS IN SUPABASE CREDENTIALS DB SPEICHERN** (AUTO-SYNC)
   - ✅ **JEDES SECRET** aus `.claude/` → Supabase Secrets Table
   - ✅ **TABLE**: `app_secrets` oder `credentials`
   - ✅ **COLUMNS**: `key`, `value`, `environment`, `project_id`
   - ✅ **SUPABASE CLI**: `supabase secrets set <KEY>=<VALUE>`

4. **SYNCHRONISATIONS-LOGIK**
   ```javascript
   async function syncSecrets() {
     const secrets = loadFromClaudeDir();

     // 1. Vercel Sync
     for (const [key, value] of Object.entries(secrets)) {
       await exec(`vercel env add ${key} ${value} --force`);
     }
     ...
     // 3. Log
     console.log(`✅ Synced ${secrets.length} secrets to Vercel + Supabase`);
   }
   ```

---

### 🛠️ **GLOBAL CONFIGURATION PRIORITY - /start COMMAND** (MANDATORY)

**🚨 CLAUDE MUSS IMMER BEFOLGEN:**

**PROBLEM:**
- Projekte haben eigene `.claude/` Verzeichnisse mit Konfigs
- Diese werden veraltet und verursachen Inkonsistenzen
- ^ CLAUDE MUSS IMMER die globalen Konfigs aus `~/.claude/` verwenden

**LÖSUNG:**
- **Global Directory**: `~/.claude/` = **Source of Truth**
- **Project Directory**: `PROJECT_ROOT/.claude/` = **Auto-synced Cache**
- **Command**: `/start` synchronisiert automatisch

**MANDATORY WORKFLOW:**

1. **Vor JEDEM größeren Task:**
   ```bash
   /start  # oder: node .claude/config-sync.js
   ```

2. **Was passiert:**
   - ✅ **Global → Project**: `~/.claude/` → `./.claude/`
   - ✅ **Backups**: Automatische Sicherung vor Überschreiben
   - ✅ **Preserve**: Projekt-spezifische Files bleiben erhalten
   - ✅ **Scripts**: Alle Executor-Scripts werden aktualisiert
   - ✅ **Notice**: `GLOBAL_CONFIG_NOTICE.md` wird erstellt

3. **Behandelte Files (immer von global):**
   - `GLOBAL_INFRASTRUCTURE.md` - Alle Geheimnisse & Infrastruktur
   - `QUICK_REFERENCE.md` - Command Reference
   - `CLAUDE_USAGE_GUIDE.md` - Usage Instructions
   - `MASTER_DEVELOPER_LOOP.md` - Komplette Architecture
   - `HYBRID_APPROACH_COMPLETE.md` - Vision Gate Dokumentation
   - Alle Executor-Scripts (`auto-swarm-executor.js`, etc.)

4. **Erhaltene Files (project-spezifisch):**
   - `PROJECT_KNOWLEDGE.md`
...
**WANN VERWENDEN:**
- ✅ Nach Updates der globalen Konfiguration
- ✅ Wenn Projekt zurückfällt auf verbesserte Workflows
- ✅ Vor Start neuer Entwicklungsaufgaben
- ✅ Regelmäßig (wöchentlich) für Updates

**⚠️ WICHTIGE REGELN:**
1. **NIE** globale Files im Projekt-Verzeichnis direkt bearbeiten
2. **IMMER** `/start` ausführen nach globalen Änderungen
3. **NUR** project-spezifische Files in `./.claude/` bearbeiten
4. **Global** hat immer Priorität über Project

**🎯 ZUSAMMENFASSUNG:**
```
~/.claude/ (Global) → /start → PROJECT/.claude/ (Auto-synced)
      ↑ (Edit here)              ↓ (DO NOT EDIT)
      global source              project cache
```
...
### 2.2 Netflix Conductor Core Features Integration (PARALLEL AGENTEN-SCHWARM + CHROME DEVTOOLS MCP + BIG PICKLE)
...
graph TD
    ...
    P -->|Ja| R[🎨 CHROME DEVTOOLS MCP<br/>Visual Analysis]

    R --> R1[📸 Full Page Screenshot<br/>+ Console + Network]
    R --> R2[🔍 Element Inspector<br/>DOM + CSS Computed]
    R --> R3[📱 Responsive Check<br/>Mobile Viewports]

    R1 --> S{Visual Score ≥ 8.5?}
    R2 --> S
    R3 --> S

    S -->|Nein| T[Auto-Fix UI<br/>Tailwind CSS + Grid]
...
- 🎨 **Chrome DevTools MCP**: Visual Analysis (Screenshot + Console + DOM)
- 🚀 **Deploy**: Vercel Live (Auto)

**🎨 Chrome DevTools MCP Superpowers:**
```
✅ Full Page Screenshots
✅ Element Snapshots (DOM + CSS)
✅ Layout Debugging (Grid/Flexbox)
